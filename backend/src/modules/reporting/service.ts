...
/**
 * Reporting module — read-only aggregator.
 * Pre-aggregates time-series data using date_trunc() GROUP BY queries.
 * No materialized views — per full-stack-design §3.2, direct queries
 * are sufficient at this data volume.
 */

type GroupBy = 'day' | 'week' | 'month';

function getDateTrunc(groupBy: GroupBy): string {
  switch (groupBy) {
    case 'day': return 'day';
...
export async function getProductionReport(params: {
  ...
  to?: string;
  groupBy?: string;
  filterBy?: string; // finishedGoodId
}): Promise<{ series: Array<{ period: string; unitsProduced: number; jobsCompleted: number; avgCompletionDays: number }> }> {
  const groupBy = (params.groupBy || 'day') as GroupBy;
  ...
  const runsResult = await prisma.$queryRaw<Array<{ period: Date; units_produced: bigint }>>`
  ...
  const jobsResult = await prisma.$queryRaw<Array<{ period: Date; jobs_completed: bigint; avg_days: number | null }>>`
  ...
  const periodMap = new Map<string, { unitsProduced: number; jobsCompleted: number; avgCompletionDays: number }>();
...
export async function getInventoryReport(params: {
  ...
  groupBy?: string;
  itemId?: string;
}): Promise<{ series: Array<{ period: string; consumed: number; received: number; netChange: number }> }> {
  const groupBy = (params.groupBy || 'day') as GroupBy;
  const trunc = getDateTrunc(groupBy);
...
export async function getOrdersReport(params: {
  ...
  groupBy?: string;
  customerId?: string;
}): Promise<{ series: Array<{ period: string; orderCount: number; onTimeRate: number; cancelledCount: number }> }> {
  const groupBy = (params.groupBy || 'day') as GroupBy;
  const trunc = getDateTrunc(groupBy);
